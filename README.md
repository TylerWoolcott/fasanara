...
Makes most sense to use **React**, **NextJS**, and **TailwindCSS**, for the frontend because:
 ...
 - They can work in combination. I built each part of the page using reusable components. When the logic/code in a particular section grew big enough, such as for the Fasanara fund cards, I broke them into separate components. 
 - If the page were to expand into a full-fledged website, as I've stubbed it out in the `NavBar` file, it would be straighforward to implement React's routing, state management, data-binding, etc. 
 - NextJS enables a more performant user interfact. For example, code splitting which reduces the size of the app’s first payload by splitting the code and serving components only when needed and built-in image optimization, as well as easy creation of API endpoints as a Node.js serverless function if the website grew over time. 
 ...
 - The navigation bar can be toggled on smaller screens and mobile devices. This is achieved with React's `useState` hook and TailwindCSS styling. 
